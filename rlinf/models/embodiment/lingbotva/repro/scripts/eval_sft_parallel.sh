#!/bin/bash
# Eval SFT baseline (step 0) across all 10 tasks x 5 episodes on 8 GPUs.
set -euo pipefail
...
OUT_DIR="${1:-/workspace/rl_logs/eval_sft}"
...
for task_id in "${TASKS[@]}"; do
    for ep_idx in "${EPS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$gpu_idx "$PY" "$WORKER" "$task_id" "$ep_idx" "$OUT_DIR" sft \
            > "$logfile" 2>&1 &
        pids+=($!)
...
<title>LingBot-VA SFT baseline (step 0) eval — LIBERO-Object</title>
<style>
  body {{background:#111;color:#ddd;font-family:monospace;margin:0;padding:12px}}
  h1 {{margin:8px 0 4px}}
  .nav {{margin-bottom:14px;font-size:13px}}
  .nav a {{color:#7af;text-decoration:none;border:1px solid #7af;padding:3px 10px;border-radius:3px}}
...
<h1>LingBot-VA SFT baseline (step 0) — LIBERO-Object eval</h1>
<div class="nav">&#8594; <a href="../eval_all_tasks_combined/index.html">GRPO step-170</a></div>
<div class="summary">
  <b>Overall SR: {total_ok}/{total} ({sr_pct:.1f}%)</b> &nbsp;|&nbsp; top row = observation, bottom row = imagination/diffusion
  <table style="margin-top:8px">
    <tr><th>id</th><th>task</th><th>SR</th></tr>
    {''.join(summary_rows)}
  </table>
</div>
